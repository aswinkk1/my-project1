...
.directive('outsideClickHandler',['$window', function($window) {
  return {
    ...
    link: function(scope, element) {
      ...
      var hasTouchEvents = 'ontouchstart' in window || 'onmsgesturechange' in window; 
      if(hasTouchEvents) {
        w.on('touchend', emitEvent);
      }
      else {
        w.on('click', emitEvent);        
      }
...
.directive('scrollbar', ['$window', '$timeout', '$parse', '$animate', 'scrollbar', function($window, $timeout, $parse, $animate, scrollbar) {
    var getTemplate = function(contentType) {
        ...
        if(window.matchMedia('(min-width: 1025px)').matches) {
          ...
          template = '<div class="scroll-viewport-touch" ng-transclude></div>';
        }
        return template;
    }
    return {
